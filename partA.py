...
def help(func, start, end,n):
    ...
    while round(start,2) < end:
        if f(start) * f(next) < 0:
            if n == 1:
                print("x = {}00000131252".format(Newton_Raphson(func,func_tag, start, next)))
                print("")
            if n == 2:
                print("x = {}00000131252".format(secant_method(func, start, next)))
                print("")
        if f(start) == 0:
            print("num of iteration 0")
            print("x = {}0000131252".format(float(start)))
            print("")
...
def main():
    y = sp.sin
    f = y(x ** 2 + 5 * x + 6) / (2 * pow(math.e, (-1) * x))
    start = -1.5
    end = 2
    print("\n````````````````````````Newton Raphson````````````````````````\n")
    help(f, start, end, 1)
    ...
    print("S={}00000131330".format(round(SimpsonRule(f,0,1,6),3)))
    print("\n````````````````````````Rombergs Method, Integral Value in Countries[0,1]````````````````````````\n")
    print("S={}00000131330".format(round(RombergsMethod(f,0,1,4),3)))
